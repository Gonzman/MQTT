<template>
    ...
    <UButton @click="createNode">Create Node</UButton>
</template>
...
async function createNode() {
    ...
    if (selected.value.id == "CHANGEUUID") {
        ...
        const requestTopic = await client.POST("/topics", { body: { mqttTopic: selected.value.mqttTopic } })
    ...
    const request = await client.POST("/categories/{categoryId}/nodes", {
        params: { path: { categoryId: props.category } }, body: {
            name: name.value, topicId: topic.id
        }
    })
